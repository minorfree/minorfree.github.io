...
Clearly \\(|E(H)| = O_{\epsilon,d}(n)\\) given an \\(\epsilon\\)-LSO with \\(O_{\epsilon,d}(1)\\) orderings. Showing that the distance is preserved up to \\((1+\epsilon)\\) is bit tricker and induction will work. 
...
\begin{split}
d_H(x,y) &\leq d_H(x,z_x) + d_H(z_x,z_y) + d_H(z_y,y) \\ 
&\leq \left((1+5\epsilon)2\epsilon + (1+2\epsilon)\right) \lVert xy\rVert_2 \\ & \leq (1+5\epsilon) \lVert xy\rVert_2
\end{split}
