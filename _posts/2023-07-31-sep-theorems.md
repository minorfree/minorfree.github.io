...
Otherwise, let \\(\bar{L}_t = \{\bar{v}\in \bar{G}: d_\bar{G}(R,\bar{v})\} = t\\) for every $1\leq t\leq k\ell$. We refer to each \\(\bar{L}_t\\) as a level. Observe that removing any \\(\bar{L}_t\\) from \\({\bar G}\\) will disconnect \\(R\\) from \\(S\\). Let \\(t^{\ast}\in [1,k\ell]\\) be the level of minimum size;  that is, \\(\lvert\bar{L}_{t^{\ast}}\rvert= \min_{1\leq t\leq k\ell}\lvert \bar{L}_t\lvert\\). Let \\(X\\) be the vertices of \\(G\\) corresponding to \\(\bar{L}_{t^{\ast}}\\). Then:
