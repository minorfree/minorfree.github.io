...
**Proof of Lemma 2:** Construct \\(k\\) copies of \\(G\\), denoted by \\(G_1,G_2,\ldots, G_k\\). For each \\(i\in [1,k-1]\\), and each \\(v\in A_i\\), we connect \\(v\\)'s copy in \\(G_{i}\\) to its copy in \\(G_{i+1}\\). Let the resulting graph be \\(\bar{G}\\). Let \\(R\\) be the copy of \\(A_1\\) in \\(G_1\\) and \\(S\\) be the copy of \\(A_k\\) in \\(G_{k}\\). See Figure 4.

![](/assets/figs/ASTTreeSep.svg)

*Figure 4: (a) Graph \\(G\\) with three sets \\(A_1,A_2,A_3\\), (b) graph \\(\bar{G}\\), the path \\(\bar{P}\\) and its projectin in \(c\).*

If \\(d_{\bar{G}}(R,S)\leq k\cdot \ell\\), then there is a path \\(\bar{P}\\) of length at most \\(k\cdot \ell\\) from a vertex \\(x\in R\\) to a vertex \\(y\in S\\) in \\(\bar{G}\\). Let \\(P\\) be a *projection* of \\(\bar{P}\\) in \\(G\\), which is defined as follows: if \\((\bar{u},\bar{v})\in \bar{P}\\) where \\(\bar{u}\\) and \\(\bar{v}\\) are copies of two vertices \\(u,v\\) in \\(G\\), respectively, then we add \\((u,v)\\) to \\(P\\) (if \\(\bar{u}\\) and \\(\bar{v}\\) are copies of the same vertex, we do nothing.). Clearly \\(P\\) is a connected subgraph of \\(G\\) containing at most \\(k\ell\\) edges and such that \\(P\cap A_i\not=\emptyset\\). Then any spanning tree \\(T\\) of \\(P\\) will satisfy the lemma.

Otherwise, let \\(\bar{L}_{t} = \{\bar{v}\in \bar{G}: d_\bar{G}(R,\bar{v})\} = t\\) for every \\(1\leq t\leq k\ell\\). We refer to each \\(\bar{L}_{t}\\) as a level. Observe that removing any \\(\bar{L}_{t}\\) from \\({\bar G}\\) will disconnect \\(R\\) from \\(S\\). Let \\(t^{\ast}\in [1,k\ell]\\) be the level of minimum size;  that is, \\(\lvert\bar{L}_{t^{\ast}}\rvert= \min_{1\leq t\leq k\ell}\lvert \bar{L}_{t}\lvert\\). Let \\(X\\) be the vertices of \\(G\\) corresponding to \\(\bar{L}_{t^{\ast}}\\). Then:

$$\lvert X\rvert \leq \lvert \bar{L}_{t^{\ast}}\rvert \leq \frac{\lvert V(\bar{G})\rvert} {k\ell} = \frac{n}{\ell}$$

We claim that no connected component \\(C\subseteq G\setminus X\\) intersects all \\(A_i\\). Suppose otherwise, then we form a path \\(Q\\) composing of \\(k\\) paths \\(Q_1,Q_2,\ldots, Q_{k-1}\\) as follows: \\(Q_1\\) is a path in \\(C\\) from  an arbitrary vertex \\(v\in A_1\\) to another (arbitrary) vertex in \\(A_2\\), and for every other \\(i\in [2,k-1]\\),  \\(Q_i\\) is a path in \\(C\\) from  the endpoint of \\(Q_{i-1}\\) to an (arbitrary) vertex \\(u \in A_{i+1}\\). We then can map \\(Q\\) back to a path \\(\bar{Q}\\) from a vertex in \\(R\\) to a vertex in \\(S\\) in \\(\bar{G}\\) in a natural way. Moreover, \\(Q\\) does not contain any vertex of \\(\bar{L}_{t^{\ast}}\\), contradicting that removing \\(\bar{L}_{t^{\ast}}\\) from \\(\bar{G}\\) disconnects \\(R\\) and \\(S\\).
